...
{
  const Math::Vector3 vector (0.1, 2.3, 3.7);
...
TEST (Vector3Test, index_operator_throws_index_operator_out_of_range_exception)
{
  ...
  EXPECT_THROW (vector[3], Math::Vector3::index_operator_out_of_range_exception);
}
...
{
  Math::Vector3 vector;
  EXPECT_THROW (vector.normalize (), Math::Vector3::normalizing_zero_vector_exception);
}
...
{
  const auto vector = create_random_vector3 ();
  EXPECT_THROW (vector / 0.0, Math::Vector3::division_by_zero_exception);
}
...
TEST (Vector3Test, equality_operator_returns_true_for_objects_with_equal_elements)
{
  ...
  const Math::Vector3 vector (array);
  const Math::Vector3 other (array);
...
{
  const Math::Vector3 vector (3.2, 5.4, 1.2);
  const Math::Vector3 other (5.6, 5.4, 1.2);
...
{
  const Math::Vector3 vector (3.2, 5.4, 1.2);
  const Math::Vector3 other (3.2, 1.4, 1.2);
  EXPECT_FALSE (vector == other);
}
...
{
  const Math::Vector3 vector (3.2, 5.4, 1.2);
  const Math::Vector3 other (3.2, 5.4, 3.7);
  EXPECT_FALSE (vector == other);
}
...
{
  const Math::Vector3 vector (3.2, 5.4, 1.2);
  const Math::Vector3 other (3.2, 5.4, 1.2);
  EXPECT_FALSE (vector != other);
}
...
{
  Math::Vector3 vec1 (3.0, 5.0, 7.0);
  Math::Vector3 vec2 (7.0, 9.0, 17.0);
  Math::Vector3 zero (0.0, 0.0, 0.0);

  EXPECT_THROW (Math::Vector3::generateOrthonormalBasis (zero, vec2, vec1), Math::Vector3::can_not_make_orthonormal_basis_with_zero_vector_exception);
  EXPECT_THROW (Math::Vector3::generateOrthonormalBasis (vec1, zero, vec2), Math::Vector3::can_not_make_orthonormal_basis_with_zero_vector_exception);
  EXPECT_THROW (Math::Vector3::generateOrthonormalBasis (vec1, vec2, zero), Math::Vector3::can_not_make_orthonormal_basis_with_zero_vector_exception);
}
...
TEST (Vector3Test, orthonormal_basis_with_equal_vectors_throws_can_not_make_orthonormal_basis_with_equal_vectors_exception)
{
  Math::Vector3 vec1 (3.0, 5.0, 7.0);
  Math::Vector3 vec2 (7.0, 9.0, 17.0);
  EXPECT_THROW (Math::Vector3::generateOrthonormalBasis (vec1, vec2, vec1), Math::Vector3::can_not_make_orthonormal_basis_with_equal_vectors_exception);
  EXPECT_THROW (Math::Vector3::generateOrthonormalBasis (vec1, vec2, vec2), Math::Vector3::can_not_make_orthonormal_basis_with_equal_vectors_exception);
  EXPECT_THROW (Math::Vector3::generateOrthonormalBasis (vec1, vec1, vec2), Math::Vector3::can_not_make_orthonormal_basis_with_equal_vectors_exception);
}
...
TEST (Vector3Test, orthonormal_basis_gives_vectors_of_length_1)
{
  ...
  Math::Vector3::generateOrthonormalBasis (vec1, vec2, vec3);
