...
{
  EXPECT_EQ(0.0, length(Math::Vec3d::ZERO));
  EXPECT_EQ(0.0, lengthSquared(Math::Vec3d::ZERO));
}

TEST_F(Vector3Test, length_of_vector_follows_mathematical_rules)
{
  EXPECT_EQ(std::sqrt(random_vector[0]*random_vector[0] + random_vector[1]*random_vector[1] + random_vector[2]*random_vector[2]), length(random_vector));
  EXPECT_EQ((random_vector[0]*random_vector[0] + random_vector[1]*random_vector[1] + random_vector[2]*random_vector[2]), lengthSquared(random_vector));
}
...
{
  EXPECT_DEATH(random_vector /= (double) 0, "Can not divide vector by 0");
}
...
{
  auto res = cross(random_vector, random_vector2);
...
{
  EXPECT_EQ(0.0, dot(random_vector, Math::Vec3d::ZERO));
}
...
{
  generateOrthonormalBasis(random_vector, random_vector2, random_vector3);

  EXPECT_FLOAT_EQ(1, length(random_vector));
  EXPECT_FLOAT_EQ(1, length(random_vector2));
  EXPECT_FLOAT_EQ(1, length(random_vector3));
}

TEST_F(Vector3Test, orthonormalizing_with_zero_vector_asserts)
{
  EXPECT_DEATH(generateOrthonormalBasis(empty_vector, random_vector, random_vector2), "Can not make orthonormal basis with zero vectors");
  EXPECT_DEATH(generateOrthonormalBasis(random_vector, empty_vector, random_vector2), "Can not make orthonormal basis with zero vectors");
  EXPECT_DEATH(generateOrthonormalBasis(random_vector, random_vector2, empty_vector), "Can not make orthonormal basis with zero vectors");
}

TEST_F(Vector3Test, ortonormalizing_with_equal_vectors_asserts)
{
  EXPECT_DEATH(generateOrthonormalBasis(random_vector, random_vector, random_vector), "Can not make orthonormal basis with equal vectors");
  EXPECT_DEATH(generateOrthonormalBasis(random_vector2, random_vector, random_vector), "Can not make orthonormal basis with equal vectors");
  EXPECT_DEATH(generateOrthonormalBasis(random_vector, random_vector2, random_vector), "Can not make orthonormal basis with equal vectors");
  EXPECT_DEATH(generateOrthonormalBasis(random_vector, random_vector, random_vector2), "Can not make orthonormal basis with equal vectors");
}
...
TEST_F(Vector3Test, orthonormal_basis_gives_dot_products_of_zero)
{
  ...
  EXPECT_NEAR(0, dot(random_vector, random_vector2), PRECISION);
  EXPECT_NEAR(0, dot(random_vector, random_vector3), PRECISION);
  EXPECT_NEAR(0, dot(random_vector2, random_vector3), PRECISION);
}
