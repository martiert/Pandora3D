...
TEST_F(Matrix3Test, matrix_determinant_follows_mathematical_rules)
{
  ...
  EXPECT_EQ(det, matrix_determinant(random_matrix));
}
...
TEST_F(Matrix3Test, adjugate_of_identity_is_identity)
{
  Math::Matrix3d identity;
  auto adjugate = matrix_adjugate(identity);

  for (size_t i = 0; i < 9; ++i)
    EXPECT_EQ(identity[i], adjugate[i]);
}
...
TEST_F(Matrix3Test, matrix_multiplied_with_its_inverse_from_left_is_identity)
{
  auto inverse = matrix_inverse(random_matrix);
